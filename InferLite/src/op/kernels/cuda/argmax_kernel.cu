...
size_t argmax_kernel_cu(const float* input_ptr, size_t size, cudaStream_t stream) {
  ...
  argmax_kernel_fp32<<<1, 512, 0, stream>>>(input_ptr, size, index);
  cudaMemcpyAsync(&output_index, index, sizeof(size_t), cudaMemcpyDeviceToHost, stream);
  // 回读 host 结果前必须等待流执行完毕。
